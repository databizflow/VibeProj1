...
address = st.text_input("지역명 (예: 경기 화성시 병점동)", "경기 화성시 병점동")
            ...
            if response.status_code == 200:
                try:
                    ...
                    if markers and isinstance(markers, list) and len(markers) > 0:
                        ...
                        for idx, row in df.iterrows():
                            ...
                            if detail_resp.status_code == 200:
                                ...
                                if danji_list:
                                    ...
                                    # 거래금액이 없으면 '미등록'으로
                                    if not price or price.strip() == "" or price == "0":
                                        price = "미등록"
                                    recent_prices.append(price)
                                    recent_dates.append(date)
                                else:
                                    recent_prices.append("미등록")
                                    recent_dates.append("")
                            else:
                                recent_prices.append("미등록")
                                recent_dates.append("")
                        ...
                        # 거래금액 NaN/None/'nan'을 미등록으로 치환
                        df["최근거래금액(만원)"] = df["최근거래금액(만원)"].replace({None: "미등록", "nan": "미등록"})
                        df["최근거래금액(만원)"] = df["최근거래금액(만원)"].fillna("미등록")
                        df["최근거래금액(만원)"] = df["최근거래금액(만원)"].astype(str)
                        st.success(f"총 {len(df)}건의 거래 데이터가 조회되었습니다.")
                        st.dataframe(df)
                        m = folium.Map(location=[y, x], zoom_start=15)
                        for idx, row in df.iterrows():
                            lat = str(row["위도"]).strip()
                            lng = str(row["경도"]).strip()
                            apt_nm = row["단지명"]
                            price = str(row["최근거래금액(만원)"]).strip()
                            date = str(row["최근계약일"]).strip()
                            if not price or price == "미등록" or price.lower() == "nan":
                                price_text = "미등록"
                            else:
                                price_text = f"{price}만원"
                            if not date or date.lower() == "nan":
                                date = "정보 없음"
                            popup_text = f"{apt_nm}<br>거래금액: {price_text}<br>계약일: {date}"
                            if lat and lng and lat.lower() != "nan" and lng.lower() != "nan":
